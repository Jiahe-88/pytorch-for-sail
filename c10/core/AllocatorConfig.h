...
#ifdef USE_PPU
// "small" allocations are packed in 8 MiB blocks for PPU driver
...
#ifdef USE_PPU
  std::atomic<size_t> large_segment_size_{33554432}; // 32 MB by default for PPU driver
#else
  std::atomic<size_t> large_segment_size_{20971520}; // 20 MB by default
#endif// The maximum block size that is allowed to be split.

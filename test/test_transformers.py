...
class TestSDPAFailureModes(NNTestCase):
    ...
    def test_mem_eff_attention_large_seq_len_attn_mask_index_overflow(self):
        ...
        head_dim = 16
        torch.manual_seed(0)
        make_tensor = partial(torch.randn, 1, 1, seq_len, head_dim, device=device, dtype=dtype)
        query, key, value = make_tensor() * 0.1, make_tensor() * 0.1, make_tensor()
        attn_mask = torch.full((seq_len, seq_len), float("-inf"), device=device, dtype=dtype).triu_(1)
        with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
            ref = torch.nn.functional.scaled_dot_product_attention(query, key, value, is_causal=True)
